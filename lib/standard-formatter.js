...
module.exports = {
  ...
  toggleFormatOnSave: function (editor) {
    var test = "test";  
    const key = 'standard-formatter.formatOnSave'
    const options = {scope: editor.getRootScopeDescriptor()}
    const formatOnSave = atom.config.get(key, options)
    atom.config.set(key, !formatOnSave, options)
  },
